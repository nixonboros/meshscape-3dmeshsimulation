...
for _ in range(num_logs):
    height = random.uniform(3, 10)  # Adjusted for logs
    thickness = random.uniform(0.5, 1.5)  # Adjusted for logs
    base_position = (random.uniform(0, size_x), random.uniform(0, size_y), random.uniform(0, 5))
    log_node = generate_log(mesh, base_position, height, thickness)
    myscene.nodes.append(log_node)
